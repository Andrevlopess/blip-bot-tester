...
Runs fail with an error status until these two are set. No secret key is
involved — the runner creates a fresh guest visitor account per run.
...
2. For each step, the step's `input` is sent and replies are collected. Each
   expected message gets its own wait window, measured from when the previous
   message arrived — not one deadline for the whole step.
3. Once the expected number of messages has arrived, the runner waits 5 more
   ...
   in the results.
4. Each expected/actual pair is scored, and the step passes only if every
   message meets its own threshold.
5. Every step runs even after one fails, so you see the whole conversation's
...
apps/web/
  src/
    components/
      test-builder/    # authoring UI (step item, message fields, preview)
